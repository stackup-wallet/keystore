...
contract UserOpWebAuthnVerifierTest is Test {
    ...
    UserOpWebAuthnVerifier public verifier;
    ...
    function setUp() public {
        verifier = new UserOpWebAuthnVerifier(address(this));
        _etchP256Verifier();
    }

    function testFuzz_validateData(bool withUserOp, uint256 index) public view {
        P256Key memory signer = _getSigner(index);
        bytes32 message = keccak256("Signed by signer");
        bytes memory signature = _webAuthnSign(uint256(signer.pk), message);
        ...
        uint256 validationData = verifier.validateData(message, data, abi.encode(signer.x, signer.y));
        assertEq(validationData, SIG_VALIDATION_SUCCESS);
    }

    function testFuzz_validateDataValidationFailed(bool withUserOp, uint256 index, bytes32 x, bytes32 y) public view {
        P256Key memory signer = _getSigner(index);
        bytes32 message = keccak256("Signed by signer");
        bytes memory signature = _webAuthnSign(uint256(signer.pk), message);
        ...
        uint256 validationData = verifier.validateData(message, data, abi.encode(x, y));
    ...
    function _webAuthnSign(uint256 privateKey, bytes32 message) internal pure returns (bytes memory signature) {
        ...
        signature = WebAuthn.encodeAuth(auth);
    }
